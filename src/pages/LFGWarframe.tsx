...
function LFGWarframe() {
  ...
  useEffect(() => {
    // Reference to the 'lfgFormWarframe' node in the database
    const lfgFormWarframeRef = ref(database, "lfgFormWarframe");
    const recentPostsQuery = query(lfgFormWarframeRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createlfgwarframepost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`lfgFormWarframe/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `lfgFormWarframe/${key}`))
        .then(() => {
