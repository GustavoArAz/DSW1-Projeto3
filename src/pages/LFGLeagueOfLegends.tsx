...
function LFGLeagueOfLegends() {
  ...
  useEffect(() => {
    ...
    const lfgFormLeagueOfLegendsRef = ref(database, "lfgFormLeagueOfLegends");
    const recentPostsQuery = query(lfgFormLeagueOfLegendsRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createlfgleagueoflegendspost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`lfgFormLeagueOfLegends/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `lfgFormLeagueOfLegends/${key}`))
        .then(() => {
...
export default LFGLeagueOfLegends;
