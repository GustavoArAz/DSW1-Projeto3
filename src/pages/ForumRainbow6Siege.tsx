...
function ForumRainbow6Siege() {
  ...
  useEffect(() => {
    // Reference to the 'forumFormRainbow6Siege' node in the database
    const forumFormRainbow6SiegeRef = ref(database, "forumFormRainbow6Siege");
    const recentPostsQuery = query(forumFormRainbow6SiegeRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createforumrainbow6siegepost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`forumFormRainbow6Siege/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `forumFormRainbow6Siege/${key}`))
        .then(() => {
...
export default ForumRainbow6Siege;
