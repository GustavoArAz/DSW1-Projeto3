...
function ForumDiabloIV() {
  ...
  useEffect(() => {
    // Reference to the 'forumFormDiabloIV' node in the database
    const forumFormDiabloIVRef = ref(database, "forumFormDiabloIV");
    const recentPostsQuery = query(forumFormDiabloIVRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createforumdiabloivpost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`forumFormDiabloIV/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `forumFormDiabloIV/${key}`))
        .then(() => {
