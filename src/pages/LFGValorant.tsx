...
function LFGValorant() {
  ...
  useEffect(() => {
    // Reference to the 'lfgFormValorant' node in the database
    const lfgFormValorantRef = ref(database, "lfgFormValorant");
    const recentPostsQuery = query(lfgFormValorantRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createlfgvalorantpost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`lfgFormValorant/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `lfgFormValorant/${key}`))
        .then(() => {
