...
function LFGOverwatch2() {
  ...
  useEffect(() => {
    // Reference to the 'lfgFormOverwatch2' node in the database
    const lfgFormOverwatch2Ref = ref(database, "lfgFormOverwatch2");
    const recentPostsQuery = query(lfgFormOverwatch2Ref, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createlfgoverwatch2post");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`lfgFormOverwatch2/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `lfgFormOverwatch2/${key}`))
        .then(() => {
