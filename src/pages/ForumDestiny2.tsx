...
function ForumDestiny2() {
  ...
  useEffect(() => {
    // Reference to the 'forumFormDestiny2' node in the database
    const forumFormDestiny2Ref = ref(database, "forumFormDestiny2");
    const recentPostsQuery = query(forumFormDestiny2Ref, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createforumdestiny2post");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`forumFormDestiny2/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `forumFormDestiny2/${key}`))
        .then(() => {
