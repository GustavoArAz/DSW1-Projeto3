...
function ForumHelldivers2() {
  ...
  useEffect(() => {
    // Reference to the 'forumFormHelldivers2' node in the database
    const forumFormHelldivers2Ref = ref(database, "forumFormHelldivers2");
    const recentPostsQuery = query(forumFormHelldivers2Ref, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createforumhelldivers2post");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`forumFormHelldivers2/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `forumFormHelldivers2/${key}`))
        .then(() => {
...
export default ForumHelldivers2;
