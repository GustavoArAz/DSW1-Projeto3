...
function LFGApexLegends() {
  ...
  useEffect(() => {
    // Reference to the 'lfgFormApexLegends' node in the database
    const lfgFormApexLegendsRef = ref(database, "lfgFormApexLegends");
    const recentPostsQuery = query(lfgFormApexLegendsRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createlfgapexlegendspost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`lfgFormApexLegends/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `lfgFormApexLegends/${key}`))
        .then(() => {
