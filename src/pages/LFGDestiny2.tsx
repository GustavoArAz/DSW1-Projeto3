...
function LFGDestiny2() {
  ...
  useEffect(() => {
    // Reference to the 'lfgFormDestiny2' node in the database
    const lfgFormDestiny2Ref = ref(database, "lfgFormDestiny2");
    const recentPostsQuery = query(lfgFormDestiny2Ref, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createlfgdestiny2post");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`lfgFormDestiny2/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `lfgFormDestiny2/${key}`))
        .then(() => {
