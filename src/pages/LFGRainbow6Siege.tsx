...
function LFGRainbow6Siege() {
  ...
  useEffect(() => {
    // Reference to the 'lfgFormRainbow6Siege' node in the database
    const lfgFormRainbow6SiegeRef = ref(database, "lfgFormRainbow6Siege");
    const recentPostsQuery = query(lfgFormRainbow6SiegeRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createlfgrainbow6siegepost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`lfgFormRainbow6Siege/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `lfgFormRainbow6Siege/${key}`))
        .then(() => {
...
export default LFGRainbow6Siege;
