...
function LFGDiabloIV() {
  ...
  useEffect(() => {
    // Reference to the 'lfgFormDiabloIV' node in the database
    const lfgFormDiabloIVRef = ref(database, "lfgFormDiabloIV");
    const recentPostsQuery = query(lfgFormDiabloIVRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createlfgdiabloivpost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`lfgFormDiabloIV/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `lfgFormDiabloIV/${key}`))
        .then(() => {
