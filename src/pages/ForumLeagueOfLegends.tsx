...
function ForumLeagueOfLegends() {
  ...
  useEffect(() => {
    // Reference to the 'forumFormLeagueOfLegends' node in the database
    const forumFormLeagueOfLegendsRef = ref(
      database,
      "forumFormLeagueOfLegends"
    );
    const recentPostsQuery = query(forumFormLeagueOfLegendsRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createforumleagueoflegendspost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`forumFormLeagueOfLegends/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `forumFormLeagueOfLegends/${key}`))
        .then(() => {
...
export default ForumLeagueOfLegends;
