...
function ForumFinalFantasyXIV() {
  ...
  useEffect(() => {
    // Reference to the 'forumFormFinalFantasyXIV' node in the database
    const forumFormFinalFantasyXIVRef = ref(
      database,
      "forumFormFinalFantasyXIV"
    );
    const recentPostsQuery = query(forumFormFinalFantasyXIVRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createforumfinalfantasyxivpost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`forumFormFinalFantasyXIV/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `forumFormFinalFantasyXIV/${key}`))
        .then(() => {
...
export default ForumFinalFantasyXIV;
