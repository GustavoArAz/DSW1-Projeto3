...
function ForumWarframe() {
  ...
  useEffect(() => {
    // Reference to the 'forumFormWarframe' node in the database
    const forumFormWarframeRef = ref(database, "forumFormWarframe");
    const recentPostsQuery = query(forumFormWarframeRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createforumwarframepost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`forumFormWarframe/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `forumFormWarframe/${key}`))
        .then(() => {
