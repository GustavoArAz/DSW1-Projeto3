...
function ForumOverwatch2() {
  ...
  useEffect(() => {
    // Reference to the 'forumFormOverwatch2' node in the database
    const forumFormOverwatch2Ref = ref(database, "forumFormOverwatch2");
    const recentPostsQuery = query(forumFormOverwatch2Ref, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createforumoverwatch2post");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`forumFormOverwatch2/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `forumFormOverwatch2/${key}`))
        .then(() => {
...
export default ForumOverwatch2;
