...
function ForumValorant() {
  ...
  useEffect(() => {
    // Reference to the 'forumFormValorant' node in the database
    const forumFormValorantRef = ref(database, "forumFormValorant");
    const recentPostsQuery = query(forumFormValorantRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createforumvalorantpost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`forumFormValorant/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `forumFormValorant/${key}`))
        .then(() => {
