...
function LFGFinalFantasyXIV() {
  ...
  useEffect(() => {
    // Reference to the 'lfgFormFinalFantasyXIV' node in the database
    const lfgFormFinalFantasyXIVRef = ref(database, "lfgFormFinalFantasyXIV");
    const recentPostsQuery = query(lfgFormFinalFantasyXIVRef, limitToLast(8));
  ...
  const handlePostClick = () => {
    navigate("/createlfgfinalfantasyxivpost");
  };
  ...
  const saveEdit = () => {
    if (updatedPost && editingPost) {
      ...
      updates[`lfgFormFinalFantasyXIV/${editingPost.key}`] = updatedPost;
  ...
  const handleDelete = (key: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      remove(ref(database, `lfgFormFinalFantasyXIV/${key}`))
        .then(() => {
...
export default LFGFinalFantasyXIV;
